#!/usr/bin/env bash
# Idle teleop mode → press ENTER to record one episode → repeat.
# Right arrow during recording = save + back to idle.
# Left arrow during recording  = discard + back to idle.
# Esc during recording         = stop everything.
# Ctrl+C in idle               = quit.
set -euo pipefail
...
while true; do
    echo ""
    echo "=== IDLE — robot following leader. Press ENTER to start recording, Ctrl+C to quit. ==="
    ...
    # Wait for ENTER (Ctrl+C exits the whole script via set -e + trap)
    read -r _
    kill "$TELEOP_PID" 2>/dev/null && wait "$TELEOP_PID" 2>/dev/null || true

    echo ""
    echo "=== RECORDING — Right arrow: save episode | Left arrow: re-record | Esc: stop ==="
    export PYNPUT_BACKEND_KEYBOARD=uinput
    PYTHONPATH=src uv run lerobot-record \
        --robot.type=bi_yams_follower \
        --teleop.type=bi_yams_leader \
        --teleop.left_arm_port="$LEFT_PORT" \
        --teleop.right_arm_port="$RIGHT_PORT" \
